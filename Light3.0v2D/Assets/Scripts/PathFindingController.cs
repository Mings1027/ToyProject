...
public class PathFindingController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (FinalNodeList.Count != 0)
        {
            ...
            transform.position = Vector2.MoveTowards(transform.position, finalNodePos, Time.deltaTime * moveSpeed);
    ...
    public void PathFinding()
    {
        ...
        for (int i = 0; i < sizeX; i++)
        {
            for (int j = 0; j < sizeY; j++)
            {
                ...
                foreach (Collider2D col in Physics2D.OverlapCircleAll(new Vector2(i + bottomLeft.x, j + bottomLeft.y), 0.4f))
                    if (col.gameObject.CompareTag("Wall")) isWall = true;
        ...
        while (OpenList.Count > 0)
        {
            ...
            CurNode = OpenList[0];
            var openListCount = OpenList.Count;
    ...
    void OnDrawGizmos()
    {
        if (FinalNodeList.Count != 0)
        {
            var finalNodeListCount = FinalNodeList.Count - 1;
            for (int i = 0; i < finalNodeListCount; i++)
            {
